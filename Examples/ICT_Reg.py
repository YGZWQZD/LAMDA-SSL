...
from Semi_sklearn.Algorithm.Regressor.ICT import ICT
...
# sampler
train_sampler=RandomSampler(replacement=True,num_samples=2000*64)
...
model=ICT(train_dataset=train_dataset,valid_dataset=valid_dataset,test_dataset=test_dataset,
               ...
               eval_it=20,mu=1,weight_decay=5e-2,evaluation=evaluation,
               lambda_u=1,train_sampler=train_sampler,valid_sampler=valid_sampler,test_sampler=test_sampler,
               train_batch_sampler=train_batchsampler,ema_decay=0.999,warmup=1 / 64,alpha=0.5,file=f)
