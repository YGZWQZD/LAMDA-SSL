...
class CAFA(DeepModelMixin,InductiveEstimator,ClassifierMixin):
    ...
    def start_fit(self, *args, **kwargs):
        ...
        self.class_weight = torch.zeros((len(self.num_classes), 1)).to(self.device)
    ...
    def train(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        l_feature, l_output = self._network.forward(lb_X)
        u_feature, u_output = self._network.forward(w_ulb_X)
        _,s_u_output=self._network.forward(s_ulb_X)

        self._network.eval()
        l_pred_shift = compute_score(lb_X.detach(), self._network,self.eps).detach()
        u_pred_shift = compute_score(ulb_X.detach(),self._network,self.eps).detach()
        self._network.train()

        l_domain_prob = self.discriminator.forward(l_feature)
        u_domain_prob = self.discriminator.forward(u_feature)

        permidx = match_string(lb_y, u_output)
        ...
        if self.it_total  > 100:
            u_output = pseudo_label_calibration(u_output, self.class_weight)

        # ramp up exp(-5(1 - t)^2)
        coef = 1. * math.exp(-5 * (1 - min(self.it_total / (self.warmup*self.it_total), 1)) ** 2)
        ...
        # supervised loss
        cls_loss = F.cross_entropy(l_output, lb_y, reduction="none", ignore_index=-1).mean()

        adv_coef = 1. * math.exp(-5 * (1 - min(self.it_total / 8000, 1)) ** 2)
